...
def step_ID(force, threshold, Fs, min_step, max_step):
    ...
    Example
    -------------
    >>> step_begin_all, step_end_all = step_ID(force_filtered,threshold = 20, min_step = 60) #20N threshold, 60 frame min length
    ...
    #        if i_step_end - i_step_begin > min_step: #removes false positives by comparing min step_length
...
def detrend_force(filename,Fs,Fc,min_step,step_threshold = 100,plots = False):
    ...
    min_step :               Min Step Len - Minimum number of frames the step ID algorithm will consider 1 step
